...
function CheckForHealth() {
	...
	if(checkHealth != Player.GetComponent(Player_2).health){
		Debug.Log(Player.GetComponent(Player_2).health);
		Debug.Log(checkHealth);

		var currentPosition = gameObject.GetComponent(Transform).position;

		LostHealth = (checkHealth - Player.GetComponent(Player_2).health) * 0.0385;
		...
		checkHealth = Player.GetComponent(Player_2).health;
	}
}
